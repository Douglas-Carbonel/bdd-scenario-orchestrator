...
- **State/Storage**: React state + localStorage via `useBddStore` hook (app data), Supabase (auth + user profiles/roles)
- **Data**: App data (companies, sprints, scenarios, suites) is persisted in localStorage. Auth data (users, profiles, roles) lives in Supabase.
...
- `src/hooks/useBddStore.ts` — All app state (companies, sprints, scenarios, suites, test runs) via localStorage
