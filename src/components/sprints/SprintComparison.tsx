...
export function SprintComparison({ companyId, productId, getSprintComparison }: SprintComparisonProps) {
  const data = getSprintComparison(companyId, productId);
    ...
    hasData
      ? Math.round(
          data
            ...
            .reduce((s: number, d: any) => s + (d.passRate ?? 0), 0) /
  ...
  if (data.length < 2) {
    return (
      <div className="glass-card rounded-xl p-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-foreground mb-2">Comparativo indisponível</h3>
        <p className="text-muted-foreground text-sm">
          São necessárias pelo menos 2 sprints para gerar o comparativo.
          {productId && " Experimente selecionar 'Todos os produtos' para ver mais dados."}
        </p>
      </div>
    );
  }
    ...
    <div className="glass-card rounded-xl p-6 space-y-6">
      ...
      {!hasData ? (
        <div className="text-center py-8 text-muted-foreground text-sm">
          Nenhum dado de execução disponível ainda. Execute o CI com uma sprint ativa para ver a tendência.
        </div>
      ) : (
        <>
          {/* Chart */}
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="passRate" radius={[4, 4, 0, 0]} fill="hsl(var(--primary))" maxBarSize={48} />
              </BarChart>
            </ResponsiveContainer>
          </div>
                ...
                {data.map((d: any) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 pr-4 text-muted-foreground text-xs whitespace-nowrap">
                      {format(d.sprint.startDate, "dd MMM", { locale: ptBR })} –{" "}
                      {format(d.sprint.endDate, "dd MMM yy", { locale: ptBR })}
                    </td>
          ...
          {data.some((d: any) => d.dataSource === "scenarios") && (
