...
export async function generateCypressZip(
  scenarios: Scenario[],
  companies: Company[],
  sprints: Sprint[]
): Promise<Blob> {
  ...
  // Group scenarios by feature for folder organization
  const byFeature = new Map<string, Scenario[]>();
  scenarios.forEach(scenario => {
    const feature = slugify(scenario.feature);
    if (!byFeature.has(feature)) byFeature.set(feature, []);
    byFeature.get(feature)!.push(scenario);
  });

  // Generate .feature files organized by feature folder
  byFeature.forEach((featureScenarios, featureSlug) => {
    featureScenarios.forEach(scenario => {
      ...
      const filename = `${slugify(scenario.title)}.feature`;
      zip.file(`cypress/e2e/features/${featureSlug}/${filename}`, content);
    });
  });

  // Generate step definitions
  const stepsContent = generateStepDefinitions(scenarios);
  zip.file("cypress/support/step_definitions/steps.ts", stepsContent);

  // Generate cypress config
  const configContent = generateCypressConfig();
  zip.file("cypress.config.ts", configContent);

  // Generate README with instructions
  const readme = `# Cypress BDD Tests - Generated by 4QA

## Setup

\`\`\`bash
npm install -D cypress @badeball/cypress-cucumber-preprocessor @bahmutov/cypress-esbuild-preprocessor esbuild
\`\`\`
...
cypress/
  e2e/
    features/          # .feature files organized by feature
  support/
    step_definitions/  # Step definition boilerplate
